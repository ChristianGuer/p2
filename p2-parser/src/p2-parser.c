...
static ASTNode* parse_location_after_id( TokenQueue* in);
...
ASTNode *parse_id_cases(TokenQueue *input)
{
  // parse id

  ASTNode *left;
  left = parse_location_after_id(input);
  //ASTNode *left = LocationNode_new(name, NULL, line);

  // test if it is an assignment or func call
  if ( check_next_token(input, SYM, "=")){
    //confirmed assingment
    match_and_discard_next_token(input, SYM, "=");
...
static ASTNode* parse_location_after_id( TokenQueue* in) {
    int line = TokenQueue_peek(in)->line;
 ...
 if (check_next_token(in, SYM, "(")) {
        ...
        match_and_discard_next_token(in, SYM, ")");
        ASTNode* call = FuncCallNode_new(name, args, line);
        return call;
 }   
...
static ASTNode* parse_primary(TokenQueue* in) {
  ...
  //Fix This to strip extra '\' from output
  if (t->type == STRLIT) {
    Token* tok = TokenQueue_remove(in);
  ...
  if (t->type == ID) {
    ...
    ASTNode* loc = parse_location_after_id( in);
    return loc;
  }
